...
int main()
{
	...
	for (int i = 0; i < polygons.size(); i++) {
				...
				for (auto it = a.outer().begin(); it != a.outer().end(); ++it) {
					...
					myVertices.push_back(get<0>(p));
					myVertices.push_back(get<1>(p));
					myVertices.push_back(0.0f); 
				}
	}*/
	//float vertices[60*3];
	////float *vertices = new float[myVertices.size()];
	//for (int i = 0; i < myVertices.size(); i++)
	//	vertices[i] = myVertices.at(i);
	//for (int i = 0; i < myVertices.size(); i++)
	...
	for (int i = 0; i < polygons.size(); i++) {
		...
		for (auto it = a.outer().begin(); it != a.outer().end(); ++it) {
			point p = *it;
			using boost::geometry::get;
			vertices.push_back(get<0>(p));
			vertices.push_back(get<1>(p));
